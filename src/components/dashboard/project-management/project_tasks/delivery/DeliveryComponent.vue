  ...
  <v-container>
    <v-card>
      <v-card-title class="d-flex justify-space-between">
        ...
        <v-btn color="primary" class="ml-4" @click="showCreateDeliveryDialog = true">
          Tạo đơn hàng
        </v-btn>
      </v-card-title>
      ...
      <v-data-table-virtual :headers="headers" :items="deliveries" :items-per-page="pageSize">
        ...
        <template v-slot:[`item.deliveryStatus`]="{ item }">
          <v-chip>{{ getDeliveryStatus(item.deliveryStatus) }}</v-chip> 
        </template>
    ...
    <!-- Dialog thao tác đơn hàng -->
    <create-delivery-dialog v-model:show="showCreateDeliveryDialog"
      @deliveryCreated="fetchDeliveries"></create-delivery-dialog>
  </v-container>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      showCreateDeliveryDialog: false,
      statuses: [
      ...
      headers: [
        ...
        { title: 'Người giao hàng', value: 'deliverName' },
  ...
  methods: {
    ...
    fetchDeliveries() {
      apiService.getDeliveries(this.selectedStatus, this.pageNumber, this.pageSize)
        .then((response) => {
